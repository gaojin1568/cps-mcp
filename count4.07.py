...
# 提前计算好矩阵后再计算每个窗口数据的频率、有效值及相对于本窗口第一个点的延迟，添加单位，增加了128Hz的相位计算，添加直流偏置，128Hz换成640Hz
import numpy as np
import pandas as pd
import time
...
def process_window_accurate(S_window: np.ndarray, window_idx: int, start_sample: int) -&gt; Dict[str, Any]:
    ...
    t_first_absolute = np.float32(start_sample / Fs_ASSUMED)
    ...
    for i, f_nom in enumerate(ALL_FREQUENCIES_HZ):
        ...
        if f_nom in DELAY_TARGET_FREQS:
            ...
            phi_first = phi_absolute - PRE['two_pi'] * f_val * t_first_absolute
            window_calc_delay_ms = (phi_first / (PRE['two_pi'] * f_val)) * np.float32(1000.0)
            ...
            theory_freq_val = np.float32(THEORY_FREQ[f_nom])
            expected_delay_ms = np.float32(TARGET_EXPECTED_DELAY_MS[f_nom])
            window_theory_delay_ms = expected_delay_ms
            
            calc_error_ms = window_calc_delay_ms - window_theory_delay_ms
            res[f"{int(f_nom)}Hz窗口理论延迟(ms)"] = window_theory_delay_ms
            res[f"{int(f_nom)}Hz计算误差(ms)"] = calc_error_ms
